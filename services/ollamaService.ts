...
export const duplicateNode = async (nodeId: string) => {
  try {
    const response = await fetch(`${API_URL}/tree/nodes/${nodeId}/duplicate`, { method: 'POST' });
    return response.ok;
  } catch (error) {
...
export const updateNode = async (nodeId: string, name?: string, content?: string) => {
  try {
    const response = await fetch(`${API_URL}/tree/nodes/${nodeId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
