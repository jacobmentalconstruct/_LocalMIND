...
interface PromptInspectorProps {
    ...
    onCancel: () => void;
    model: string;
    isBuilding: boolean; // [NEW] Indicates if backend is still assembling the context
}

const PromptInspector: React.FC<PromptInspectorProps> = ({ promptText, onPromptChange, onRun, onCancel, model, isBuilding }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-gray-900 border-l border-gray-700 w-[500px] flex-shrink-0 transition-all">
            <div className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-800">
                <h2 className="text-lg font-bold flex items-center gap-2">
                    <BotIcon className="w-5 h-5 text-green-400"/> Prompt Inspector
                </h2>
                <div className="flex items-center gap-3">
                    {isBuilding && <span className="text-xs text-yellow-400 animate-pulse font-mono">Orchestrating...</span>}
            ...
            <div className="flex-1 relative group">
                ...
                {/* Floating Snippet Tool (Hide when building to prevent interactions) */}
                {!isBuilding && (
